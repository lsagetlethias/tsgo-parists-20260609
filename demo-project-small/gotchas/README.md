...
> Tous les diagnostics ci-dessous sont **identiques entre tsc et tsgo** (parité
> ~99.99 %). tsgo répond simplement quasi instantanément.
...
tsc  -p gotchas/esModuleInterop-removed.tsconfig.json   # TS5107 (déprécié)
tsgo -p gotchas/esModuleInterop-removed.tsconfig.json   # TS5108 (supprimé)
tsgo -p gotchas/baseUrl-removed.tsconfig.json           # TS5102 + TS5090

# 2) const enum vs type-stripping
tsc  -p gotchas/tsconfig.json   # TS1294
tsgo -p gotchas/tsconfig.json   # TS1294, en ~0s
```
...
3. Compare les `.d.ts` :
   ...
   ```bash
   tsc  -p repro.tsconfig.json && cat dist-repro/repro.d.ts
   tsgo -p repro.tsconfig.json && cat dist-repro/repro.d.ts
   ```
